    ...
    def get_cached_market_data(
        ...
        try:
            with self.db.get_session() as session:
                ...
                query = (
                    session.query(CachedMarketData)
                    .join(
                        subq,
                        and_(
                    ...
                    .order_by(desc(CachedMarketData.fetched_at))
                    .limit(limit)
                )
    ...
    def get_cached_ohlc(
        ...
        try:
            with self.db.get_session() as session:
                # Query for OHLC data
                query = (
                    session.query(CachedOHLC)
                    .filter(and_(CachedOHLC.symbol == symbol, CachedOHLC.interval == interval))
                    .order_by(desc(CachedOHLC.timestamp))
                    .limit(limit)
                )
    ...
    def cleanup_old_data(self, days: int = 7) -> Dict[str, int]:
        ...
        try:
            with self.db.get_session() as session:
                ...
                deleted = (
                    session.query(CachedMarketData)
                    .filter(CachedMarketData.fetched_at < cutoff_time)
                    .delete()
                )
                ...
                # Clean old OHLC data
                deleted = (
                    session.query(CachedOHLC).filter(CachedOHLC.fetched_at < cutoff_time).delete()
                )
                ...
                logger.info(
                    f"Cleaned up {total_deleted} old cache records (older than {days} days)"
                )
