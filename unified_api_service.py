...
async def search_resources(q: str = Query(..., min_length=2)):
    ...
    return {
        ...
        "count": len(results),
        "results": [
            {"id": r.id, "name": r.name, "category": r.category, "base_url": r.base_url}
            for r in results
        ],
    }
...
async def fetch_from_resource(
    resource: APIResource, endpoint: str = "", params: Dict = None
) -> Dict:
...
async def get_price(symbol: str):
    ...
    if coingecko:
        data = await fetch_from_resource(
            coingecko, f"simple/price?ids={symbol}&vs_currencies=usd,btc"
        )
    ...
    if binance:
        ...
        data = await fetch_from_resource(binance, f"api/v3/ticker/price?symbol={pair}")
        if data:
            return {
                "symbol": symbol,
                "source": "binance",
...
async def get_multiple_prices(symbols: str = Query(..., description="Comma-separated symbols")):
    ...
    data = await fetch_from_resource(coingecko, f"simple/price?ids={ids}&vs_currencies=usd,btc,eth")
...
async def get_trending_coins():
    ...
    return {"source": "coingecko", "data": data, "timestamp": datetime.now().isoformat()}
...
async def get_fear_greed_index():
    ...
    if cfgi:
        ...
        if data:
            return {"source": "cfgi", "data": data, "timestamp": datetime.now().isoformat()}
...
async def get_trading_pairs():
    ...
    try:
        ...
        return {"count": len(pairs), "pairs": pairs, "timestamp": datetime.now().isoformat()}
