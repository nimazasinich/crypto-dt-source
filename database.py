...
class CryptoDatabase:
    ...
    def _init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_analysis_symbol ON market_analysis(symbol)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_analysis_timestamp ON market_analysis(timestamp)"
            )
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_model_outputs_model_key ON model_outputs(model_key)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_model_outputs_created_at ON model_outputs(created_at)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_gap_audit_gap_type ON gap_filling_audit(gap_type)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_gap_audit_request_id ON gap_filling_audit(request_id)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_provider_cache_provider ON provider_cache(provider_key, endpoint)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_provider_cache_params ON provider_cache(params_hash)"
            )
    ...
    def get_latest_news(
        self, limit: int = 50, sentiment: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def update_news_sentiment(
        self, news_id: int, sentiment_score: float, sentiment_label: str
    ) -> bool:
    ...
    def get_model_outputs(
        self, symbol: Optional[str] = None, model_key: Optional[str] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
    ...
    def get_gap_fill_audit(
        self, gap_type: Optional[str] = None, request_id: Optional[str] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
            ...
            with self.get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    if audit_dict.get("providers_attempted"):
                        audit_dict["providers_attempted"] = json.loads(
                            audit_dict["providers_attempted"]
                        )
    ...
    def get_gap_fill_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                stats["success_rate"] = (
                    successful / stats["total_attempts"] if stats["total_attempts"] > 0 else 0
                )

                # Average confidence
                cursor.execute(
                    "SELECT AVG(confidence) as avg_conf FROM gap_filling_audit WHERE confidence IS NOT NULL"
                )
                ...
                stats["by_strategy"] = {
                    row["strategy_used"]: row["count"] for row in cursor.fetchall()
                }
    ...
    def get_provider_cache(
        self, provider_key: str, endpoint: str, params_hash: str
