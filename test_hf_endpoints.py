    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 401:
                ...
                print_test(
                    "Invalid token rejected", "FAIL", f"Got {response.status_code} instead of 401"
                )
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            print_test(
                "Cache statistics",
                "PASS",
                f"Market: {market_count} symbols, OHLC: {ohlc_count} candles",
            )
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if price and price > 0:
                print_test(
                    "Price is positive", "PASS", f"Example: {first_item['symbol']}=${price:.2f}"
                )
            else:
    ...
    async with httpx.AsyncClient() as client:
        for text in test_texts:
            try:
                ...
                if response.status_code != 200:
                    print_test(
                        f"Sentiment analysis", "FAIL", f"Status code: {response.status_code}"
                    )
                    continue
                ...
                if sentiment and score is not None:
                    print_test(
                        f"Sentiment: '{text[:40]}...'", "PASS", f"{sentiment} (score={score:.3f})"
                    )
                else:
