...
class FallbackConfig:
    ...
    def _parse_config_content(self, content: str) -> Dict[str, Any]:
        ...
        """
        providers = {"market_data": [], "whales": [], "blockchain": [], "news": [], "sentiment": []}
...
class APIClient:
    ...
    async def fetch_with_fallback(
        self, endpoint: str, category: str, params: Dict[str, Any] = None, normalize_fn=None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response = await self.client.get(f"{HF_BASE_URL}{endpoint}", params=params)
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "meta": {"attempted": attempted, "timestamp": datetime.utcnow().isoformat() + "Z"},
            },
        )

    async def _call_provider(
        self, provider: Dict[str, Any], endpoint: str, params: Dict[str, Any]
    ) -> Any:
        """
...
async def get_market_pairs(limit: int = Query(100, ge=1, le=500), page: int = Query(1, ge=1)):
    """
    ...
    **MUST be served by HF HTTP first**
    ...
    try:
        response = await api_client.client.get(
            f"{HF_BASE_URL}/api/market/pairs", params={"limit": limit, "page": page}
        )
        response.raise_for_status()
        data = response.json()
...
async def get_ohlc(
    symbol: str = Query(...), interval: int = Query(60), limit: int = Query(100, le=1000)
):
    """
    دریافت OHLC candles
...
async def predict_signal(model_key: str, request: Dict[str, Any]):
    """
    پیش‌بینی با مدل AI
    ...
    try:
        response = await api_client.client.post(
            f"{HF_BASE_URL}/api/models/{model_key}/predict", json=request
        )
        response.raise_for_status()
        data = response.json()
