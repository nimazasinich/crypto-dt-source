...
class FallbackConfigLoader:
    ...
    def _extract_section(self, content: str, section_name: str) -> Optional[str]:
        ...
        pattern = r"={3,}\s*" + re.escape(section_name) + r".*?={3,}"
    ...
    def _parse_explorers(self, section: str) -> Dict[str, List[ProviderConfig]]:
        ...
        logger.info(
            f"Loaded explorers: ETH={len(explorers['ethereum'])}, BSC={len(explorers['bsc'])}, TRON={len(explorers['tron'])}"
        )
        return explorers
    ...
    def _parse_rpc_nodes(self, section: str) -> Dict[str, List[str]]:
        """Parse RPC node URLs"""
        nodes = {"ethereum": [], "bsc": [], "polygon": [], "tron": []}
        ...
        # Tron
        tron_rpcs = ["https://api.trongrid.io", "https://api.tronstack.io"]
        ...
        logger.info(
            f"Loaded RPC nodes: ETH={len(nodes['ethereum'])}, BSC={len(nodes['bsc'])}, POLYGON={len(nodes['polygon'])}"
        )
        return nodes
