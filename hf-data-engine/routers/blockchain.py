...
async def get_eth_transactions(
    ...
    try:
        result = await provider.get_transactions(address=address, page=page, offset=offset)
        return result
...
async def get_eth_token_transfers(
    ...
    try:
        result = await provider.get_token_transfers(
            address=address, contract_address=contract, page=page, offset=offset
        )
        return result
    except Exception as e:
...
async def get_eth_balance(address: str = Query(..., description="Ethereum address")):
...
async def get_bsc_balance(address: str = Query(..., description="BSC address")):
...
async def get_tron_transactions(
    ...
    try:
        result = await provider.get_transactions(address=address, start=start, limit=limit)
        return result
    except Exception as e:
...
async def get_tron_token_transfers(
    ...
    try:
        result = await provider.get_trc20_transfers(
            address=address, start=start, limit=limit, contract_address=contract
        )
        return result
    except Exception as e:
...
async def get_tron_account(address: str = Query(..., description="TRON address")):
    """
...
async def get_tron_token_list(limit: int = Query(20, ge=1, le=50, description="Number of tokens")):
...
async def blockchain_health():
    ...
    return {
        "success": True,
        "providers": {
            "etherscan": {"name": eth.name, "baseUrl": eth.base_url, "timeout": eth.timeout},
            "bscscan": {"name": bsc.name, "baseUrl": bsc.base_url, "timeout": bsc.timeout},
            "tronscan": {"name": tron.name, "baseUrl": tron.base_url, "timeout": tron.timeout},
        },
        "endpoints": [
