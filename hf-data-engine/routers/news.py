...
async def get_news_sources(
    category: str = Query("business", description="Category filter")
):
    """
...
async def get_ethereum_news(
    ...
    try:
        result = await provider.get_latest_news(
            query="ethereum OR ETH OR vitalik",
            page_size=page_size,
            sort_by="publishedAt"
        )
        return result
