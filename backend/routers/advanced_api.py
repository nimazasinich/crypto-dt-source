        ...
        if health_log_path.exists():
            with open(health_log_path, "r", encoding="utf-8") as f:
                ...
                if recent_entries:
                    ...
                    stats["statusBreakdown"]["errors"] = sum(
                        1 for e in recent_entries if e.get("status") == "error"
                    )
                    stats["statusBreakdown"]["timeouts"] = sum(
                        1 for e in recent_entries if e.get("status") == "timeout"
                    )
            ...
            stats["requestsHistory"].append(
                {
                    ...
                    "count": max(
                        10, int(stats["totalRequests"] / 24) + (i % 5) * 3
                    ),  # Distribute evenly
                }
            )
...
async def fix_duplicates():
    ...
    try:
        ...
        if duplicates:
            # Create backup
            backup_path = (
                providers_path.parent
                / f"{providers_path.name}.backup.{int(datetime.now().timestamp())}"
            )
...
async def remove_resource(provider_id: str):
    ...
    try:
        ...
        return {"status": "success", "message": "Resource removed successfully"}
...
async def get_discovery_status():
    ...
    try:
        ...
        if not report_path.exists():
            return {"status": "not_run", "found": 0, "validated": 0, "failed": 0}
        ...
        return {"status": "error", "found": 0, "validated": 0, "failed": 0}
        ...
        for provider_id, provider_info in list(providers.items()):
            ...
            if "cryptobert" in name.lower():
                ...
                if model_key in cryptobert_models:
                    ...
                    if provider_info.get("validated", False) and not providers[existing].get(
                        "validated", False
                    ):
                        # Remove old, keep new
