...
class BinanceClient:
    ...
    async def get_ohlcv(
        ...
        try:
            ...
            if e.response.status_code == 400:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid symbol or parameters: {symbol}"
                )
            elif e.response.status_code == 404:
                logger.error(f"❌ Binance: Symbol not found: {binance_symbol}")
                raise HTTPException(
                    status_code=404, detail=f"Symbol not found on Binance: {symbol}"
                )
            else:
                logger.error(f"❌ Binance API HTTP error: {e}")
                raise HTTPException(
                    status_code=503, detail=f"Binance API temporarily unavailable: {str(e)}"
                )
            ...
            raise HTTPException(
                status_code=503, detail=f"Failed to fetch real OHLCV data from Binance: {str(e)}"
            )
    ...
    async def get_24h_ticker(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=503, detail=f"Failed to fetch ticker from Binance: {str(e)}"
            )
        except Exception as e:
            logger.error(f"❌ Binance ticker failed: {e}")
            raise HTTPException(
                status_code=503, detail=f"Failed to fetch real ticker data: {str(e)}"
            )
