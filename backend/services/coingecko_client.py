...
class CoinGeckoClient:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                if symbols:
                    ...
                    logger.info(
                        f"✅ CoinGecko: Fetched {len(prices)} real prices for specific symbols"
                    )
                    return prices
            ...
            raise HTTPException(
                status_code=503, detail=f"CoinGecko API temporarily unavailable: {str(e)}"
            )
        except Exception as e:
            logger.error(f"❌ CoinGecko API failed: {e}")
            raise HTTPException(
                status_code=503, detail=f"Failed to fetch real market data from CoinGecko: {str(e)}"
            )
    ...
    async def get_trending_coins(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=503, detail=f"CoinGecko trending API unavailable: {str(e)}"
            )
        except Exception as e:
            logger.error(f"❌ CoinGecko trending API failed: {e}")
            raise HTTPException(status_code=503, detail=f"Failed to fetch trending coins: {str(e)}")
