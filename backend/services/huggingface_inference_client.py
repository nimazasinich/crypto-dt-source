    ...
    async def analyze_sentiment(
        ...
        try:
            ...
            if not text or len(text.strip()) == 0:
                raise HTTPException(
                    status_code=400, detail="Missing or invalid text in request body"
                )
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.post(
                    f"{self.base_url}/{model_name}", headers=self.headers, json={"inputs": text}
                )
                ...
                if isinstance(data, list) and len(data) > 0:
                    ...
                    raise HTTPException(
                        status_code=500, detail="Unexpected response format from model"
                    )
            ...
            if e.response.status_code == 503:
                ...
                raise HTTPException(status_code=400, detail="Invalid text or parameters")
            else:
                logger.error(f"❌ HuggingFace API HTTP error: {e}")
                raise HTTPException(
                    status_code=503, detail=f"HuggingFace API temporarily unavailable: {str(e)}"
                )
