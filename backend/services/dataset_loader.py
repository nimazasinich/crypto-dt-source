...
class CryptoDatasetLoader:
    ...
    async def load_dataset(
        self, dataset_key: str, split: Optional[str] = None, streaming: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if dataset_key in self.datasets:
            ...
            return {
                ...
                "num_rows": (
                    len(self.datasets[dataset_key])
                    if hasattr(self.datasets[dataset_key], "__len__")
                    else "unknown"
                ),
            }
        ...
        try:
            ...
            dataset = load_dataset(
                config["dataset_id"], split=split, cache_dir=self.cache_dir, streaming=streaming
            )
    ...
    async def load_all_datasets(self, streaming: bool = False) -> Dict[str, Any]:
        ...
        for dataset_key in self.dataset_configs.keys():
            try:
                ...
                results.append({"success": False, "dataset_key": dataset_key, "error": str(e)})
    ...
    async def get_dataset_sample(
        self, dataset_key: str, num_samples: int = 10, split: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def query_dataset(
        self, dataset_key: str, filters: Optional[Dict[str, Any]] = None, limit: int = 100
    ) -> Dict[str, Any]:
        """
    ...
    def unload_dataset(self, dataset_key: str) -> Dict[str, Any]:
        ...
        """
        if dataset_key not in self.datasets:
            return {"success": False, "dataset_key": dataset_key, "message": "Dataset not loaded"}
        ...
        try:
            ...
            return {"success": False, "dataset_key": dataset_key, "error": str(e)}
