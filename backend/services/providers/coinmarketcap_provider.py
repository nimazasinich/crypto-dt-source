...
class CoinMarketCapProvider(BaseProvider):
    ...
    def _get_default_headers(self) -> Dict[str, str]:
        ...
        return {"Accept": "application/json", "X-CMC_PRO_API_KEY": self.api_key}
    ...
    async def get_quotes(
        self, symbols: Optional[str] = None, ids: Optional[str] = None, convert: str = "USD"
    ) -> Dict[str, Any]:
        """
        ...
        if not symbols and not ids:
            return create_error_response(
                self.name, "Missing parameter", "Either 'symbols' or 'ids' is required"
            )
        ...
        return create_success_response(
            self.name, {"quotes": quotes, "count": len(quotes), "convert": convert}
        )
    ...
    async def get_ohlcv_historical(
        self, symbol: str, time_period: str = "daily", count: int = 30, convert: str = "USD"
    ) -> Dict[str, Any]:
        """
    ...
    async def get_map(self, limit: int = 100) -> Dict[str, Any]:
        ...
        return create_success_response(
            self.name, {"cryptocurrencies": formatted, "count": len(formatted)}
        )
