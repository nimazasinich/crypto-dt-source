...
class HFSentimentProvider(BaseProvider):
    ...
    async def analyze_sentiment(
        self, text: str, model: Optional[str] = None, use_financial_model: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if not text or len(text.strip()) < 3:
            return create_error_response(
                self.name, "Invalid text", "Text must be at least 3 characters"
            )
        ...
        if isinstance(data, dict) and data.get("error"):
            ...
            if "loading" in error_msg.lower():
                return create_error_response(
                    self.name, "Model is loading", "Please retry in a few seconds"
                )
    ...
    async def summarize_text(
        ...
        if not text or len(text.strip()) < 50:
            return create_error_response(
                self.name, "Text too short", "Text must be at least 50 characters for summarization"
            )
    ...
    async def classify_text(
        self, text: str, candidate_labels: List[str], model: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not candidate_labels or len(candidate_labels) < 2:
            return create_error_response(
                self.name, "Invalid labels", "At least 2 candidate labels required"
            )
        ...
        payload = {
            "inputs": text,
            "parameters": {"candidate_labels": candidate_labels[:10]},  # Limit labels
        }
        ...
        for label, score in zip(labels, scores):
            classifications.append({"label": label, "score": round(score, 4)})
    ...
    async def get_available_models(self) -> Dict[str, Any]:
        ...
        return create_success_response(
            self.name, {"models": self.MODELS, "tasks": list(self.MODELS.keys())}
        )
