...
class SimpleCache:
    ...
    def cleanup_expired(self) -> int:
        ...
        now = time.time()
        expired_keys = [k for k, v in self._cache.items() if (now - v.timestamp) > v.ttl]
    ...
    async def get_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None or self._client.is_closed:
            self._client = httpx.AsyncClient(
                timeout=httpx.Timeout(self.timeout), headers=self._get_default_headers()
            )
        return self._client

    def _get_default_headers(self) -> Dict[str, str]:
        """Get default headers for requests (override in subclasses)"""
        return {"Accept": "application/json", "User-Agent": "HF-Crypto-Data-Engine/1.0"}
