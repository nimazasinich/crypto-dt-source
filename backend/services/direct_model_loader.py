...
class DirectModelLoader:
    ...
    async def load_model(self, model_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Load tokenizer
            tokenizer = AutoTokenizer.from_pretrained(config["model_id"], cache_dir=self.cache_dir)

            # Load model based on class
            if config["model_class"] == "BertForSequenceClassification":
                model = BertForSequenceClassification.from_pretrained(
                    config["model_id"], cache_dir=self.cache_dir
                )
    ...
    async def load_all_models(self) -> Dict[str, Any]:
        ...
        for model_key in self.model_configs.keys():
            try:
                ...
                results.append({"success": False, "model_key": model_key, "error": str(e)})
    ...
    async def predict_sentiment(
        self, text: str, model_key: str = "cryptobert_elkulako", max_length: int = 512
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            all_probs = {
                label_map.get(i, f"class_{i}"): probs[0][i].item() for i in range(probs.shape[1])
            }
    ...
    async def batch_predict_sentiment(
        self, texts: List[str], model_key: str = "cryptobert_elkulako", max_length: int = 512
    ) -> Dict[str, Any]:
        """
    ...
    def unload_model(self, model_key: str) -> Dict[str, Any]:
        ...
        """
        if model_key not in self.models:
            return {"success": False, "model_key": model_key, "message": "Model not loaded"}
        ...
        try:
            ...
            return {"success": False, "model_key": model_key, "error": str(e)}
