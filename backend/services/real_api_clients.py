...
class RealAPIConfiguration:
    ...
    TRONGRID_API_KEY = os.getenv(
        "TRONGRID_API_KEY", "7ae72726-bffe-4e74-9c33-97b761eeea21"
    )  # همان کلید TronScan
...
class CoinMarketCapClient:
    ...
    async def get_latest_listings(self, limit: int = 100) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=503, detail=f"Failed to fetch real market data: {str(e)}"
            )
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                return {
                    "success": True,
                    "data": data.get("data", {}),
                    "meta": {"source": "coinmarketcap", "timestamp": datetime.utcnow().isoformat()},
                }
        except Exception as e:
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                response = await client.get(
                    ...
                    headers=self.headers,
                    params={"symbol": symbol, "count": limit, "interval": interval},
                )
                response.raise_for_status()
                data = response.json()
    ...
    async def _get_ohlc_fallback(self, symbol: str, interval: str, limit: int) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                logger.info(
                    f"✅ Binance fallback: Fetched {len(ohlc_data)} real candles for {symbol}"
                )
                return {
                    "success": True,
                    "data": ohlc_data,
    ...
    async def get_top_headlines(self, limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": True,
                "articles": articles,
                "meta": {"source": "newsapi", "timestamp": datetime.utcnow().isoformat()},
            }
        except Exception as e:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.get(
                    ...
                    headers={"TRON-PRO-API-KEY": self.tronscan_key},
                )
                response.raise_for_status()
                data = response.json()
        ...
        try:
            if chain.lower() == "ethereum":
                ...
                return {
                    ...
                    "meta": {"source": "etherscan", "timestamp": datetime.utcnow().isoformat()},
                }
            else:
...
class HuggingFaceSpaceClient:
    ...
    async def get_market_data(self) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                response = await client.get(f"{self.base_url}/api/market", headers=self.headers)
                response.raise_for_status()
                data = response.json()
    ...
    async def get_trading_pairs(self) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                response = await client.get(
                    f"{self.base_url}/api/market/pairs", headers=self.headers
                )
                response.raise_for_status()
                data = response.json()
