...
async def test_health_and_diagnostics():
    ...
    if success and "tests" in data:
        ...
        print_test_result("  → Pair metadata HF source", hf_source, pair_test.get("source", "unknown"))
    else:
...
async def test_realtime_data():
    ...
    # Batch rates
    success, data, details = await make_request("GET", "/rate/batch", params={"pairs": "BTC/USDT,ETH/USDT"})
    if success:
...
async def test_pair_metadata():
    ...
    for pair in pairs_to_test:
        ...
        if success:
            meta_valid, source = validate_meta(data)
            is_hf = source == "hf"
            has_fields = all(field in data for field in ["pair", "base", "quote", "tick_size", "min_qty"])
    ...
    for params in test_cases:
        ...
        if success:
            ...
            if has_items:
                ...
                has_ohlc = all(field in first_item for field in ["open", "high", "low", "close", "volume"])
            else:
                has_ohlc = False
            
            test_name = f"GET /history ({params['symbol']}, {params['interval']}s, limit={params['limit']})"
...
async def test_market_overview():
    ...
    if success:
        has_fields = all(field in data for field in ["total_market_cap", "btc_dominance", "volume_24h"])
...
async def test_model_predictions():
    ...
    success, data, details = await make_request("POST", "/models/price_lstm/predict", json_data=predict_data)
    ...
    success, data, details = await make_request("POST", "/models/batch/predict", json_data=batch_data)
...
async def test_performance():
    ...
    perf_tests = [
        ...
        ("History (small)", "GET", "/history", {"symbol": "BTC", "interval": 60, "limit": 10}, 4000),
...
async def verify_acceptance_criteria():
    ...
    if success:
        meta_valid, source = validate_meta(data)
        criteria.append({
            "name": "Pair metadata source is HF",
            "passed": source == "hf",
            "actual": source
        })
    ...
    success, data, _ = await make_request("GET", "/history", params={"symbol": "BTC", "interval": 60, "limit": 5})
    if success:
    ...
    success, data, _ = await make_request("GET", "/whales", params={"chain": "ethereum", "min_amount_usd": 100000})
    if success:
    ...
    success, data, _ = await make_request("POST", "/models/test_model/predict", json_data=predict_data)
    criteria.append({
    ...
    for criterion in criteria:
        if criterion["passed"]:
            ...
            print(f"{Fore.RED}✗{Style.RESET_ALL} {criterion['name']}: Expected but got {criterion['actual']}")
