 ...
 */
const CRYPTOS = [
    // REMOVED: demoPrice - All prices must be fetched from real APIs
...
class TradingAssistantProfessional {
    ...
    async fetchPrice(symbol) {
        ...
        // NO FALLBACK - All sources failed
        console.error(`[API] All sources failed for ${symbol} - no price available`);
        return 0;
    }
    ...
    async fetchOHLCV(symbol, timeframe, limit) {
        ...
        // NO FALLBACK - All sources failed
        console.error(`[API] All sources failed for ${symbol} OHLCV - no data available`);
        return [];
    }
    
    // REMOVED: generateDemoOHLCV() - No demo data generation allowed, only real data from APIs
