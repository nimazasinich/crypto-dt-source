...
class SystemMonitor {
    ...
    createPacket(request) {
        ...
        if (this.clientNodes.length > 0) {
            // Use first available client node (not random)
            fromNode = this.clientNodes.length > 0 ? this.clientNodes[0] : null;
            ...
            if (endpoint.includes('models') || endpoint.includes('sentiment')) {
                ...
                // Use first available node (not random)
                returnNode = this.nodes.length > 0 ? this.nodes[0] : null;
            }
        }
    ...
    startAnimation() {
        ...
        // REMOVED: Demo packet generation - Only show real API requests
        // Real packets will be created when actual API calls are made
    }
    ...
    destroy() {
        ...
        // REMOVED: demoPacketInterval - No demo packets
        }
    }
}
