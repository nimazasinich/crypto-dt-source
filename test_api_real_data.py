...
for i, row in enumerate(market_data[:3], 1):
    ...
    print(
        f"     24h Change: {row['change_24h']:.2f}%"
        if row["change_24h"]
        else "     24h Change: N/A"
    )
...
if market_data_response and len(market_data_response) > 0:
    ...
    print(
        f"   Sample: {market_data_response[0]['symbol']} at ${market_data_response[0]['price']:.2f}"
    )
else:
...
if ohlc_data_response and len(ohlc_data_response) > 0:
    ...
    print(
        f"   Sample: Open={ohlc_data_response[0]['open']:.2f}, Close={ohlc_data_response[0]['close']:.2f}"
    )
else:
