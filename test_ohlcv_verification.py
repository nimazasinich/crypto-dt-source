...
class OHLCVVerifier:
    ...
    def verify_coingecko(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, list) and len(data) > 0:
                    ...
                    for idx, record in enumerate(data):
                        if len(record) >= 5:
                            ...
                            issues.append(
                                f"Record {idx} has insufficient fields (expected 5, got {len(record)})"
                            )
                    ...
                    result.update(
                        {
                            "success": True,
                            "status": (
                                "SUCCESS" if valid_count >= 25 and len(issues) == 0 else "PARTIAL"
                            ),
                            "valid_records": valid_count,
                            "fields_verified": required_fields,
                            "missing_data": (
                                f"{len(missing_days)} issues" if missing_days else "None"
                            ),
                ...
                result.update(
                    {"error": f"HTTP {response.status_code}", "error_details": response.text[:500]}
                )

        except requests.exceptions.Timeout:
            result.update(
                {"error": "Request timeout", "error_details": "Request exceeded 30 second timeout"}
            )
        except requests.exceptions.RequestException as e:
            result.update({"error": "Request failed", "error_details": str(e)})
        except Exception as e:
            result.update({"error": "Unexpected error", "error_details": str(e)})
                ...
                if isinstance(data, list) and len(data) > 0:
                    ...
                    for idx, record in enumerate(data):
                        if len(record) >= 7:
                            ...
                            issues.append(
                                f"Record {idx} has insufficient fields (expected at least 7, got {len(record)})"
                            )
                    ...
                    result.update(
                        {
                            "success": True,
                            "status": (
                                "SUCCESS" if valid_count >= 360 and len(issues) == 0 else "PARTIAL"
                            ),
                            "valid_records": valid_count,
                            "fields_verified": required_fields,
                            "missing_data": (
                                "None"
                                if valid_count >= 360
                                else f"Missing {365 - valid_count} records"
                            ),
                        ...
                        for timestamp, values in time_series.items():
                            if isinstance(values, dict):
                                open_price = values.get(
                                    "1a. open (USD)", values.get("1. open (USD)")
                                )
                                ...
                                try:
                                    ...
                                    if None in [open_price, high, low, close, volume]:
                                        ...
                                        issues.append(
                                            f"Record {timestamp} has inconsistent OHLC values"
                                        )
                                    elif volume < 0:
                        ...
                        result.update(
                            {
                                ...
                                "status": (
                                    "SUCCESS"
                                    if valid_count >= 50 and len(issues) == 0
                                    else "PARTIAL"
                                ),
                                "valid_records": valid_count,
                                "fields_verified": required_fields,
                                "missing_data": (
                                    "None"
                                    if valid_count >= 50
                                    else f"Only {valid_count} valid records"
                                ),
                                "issues": issues[:10] if issues else "None",
                                "sample_data": {
                                    "timestamp": sample_timestamp,
                                    "open": sample_values.get(
                                        "1a. open (USD)", sample_values.get("1. open (USD)")
                                    ),
                                    "high": sample_values.get(
                                        "2a. high (USD)", sample_values.get("2. high (USD)")
                                    ),
                                    "low": sample_values.get(
                                        "3a. low (USD)", sample_values.get("3. low (USD)")
                                    ),
                                    "close": sample_values.get(
                                        "4a. close (USD)", sample_values.get("4. close (USD)")
                                    ),
                                    "volume": sample_values.get(
                                        "5. volume", sample_values.get("5a. volume")
                                    ),
                                },
                                "raw_response_preview": {
                                    k: v for k, v in list(time_series.items())[:2]
                                },
                            }
                        )
                    else:
    ...
    def verify_twelvedata(self) -> Dict[str, Any]:
        ...
        params = {"symbol": "BTC/USD", "interval": "1min", "apikey": api_key, "outputsize": 1440}
                        ...
                        result.update(
                            {
                                ...
                                "status": (
                                    "SUCCESS"
                                    if valid_count >= 1400 and len(issues) == 0
                                    else "PARTIAL"
                                ),
                                "valid_records": valid_count,
                                "fields_verified": required_fields,
                                "missing_data": (
                                    "None"
                                    if valid_count >= 1400
                                    else f"Missing {1440 - valid_count} records"
                                ),
                                "issues": issues[:10] if issues else "None",
                        ...
                        result.update(
                            {
                                ...
                                "status": (
                                    "SUCCESS"
                                    if valid_count >= 190 and len(issues) == 0
                                    else "PARTIAL"
                                ),
                                "valid_records": valid_count,
                                "fields_verified": required_fields,
                                "missing_data": (
                                    "None"
                                    if valid_count >= 190
                                    else f"Missing {200 - valid_count} records"
                                ),
                                "issues": issues[:10] if issues else "None",
    ...
    def generate_summary_report(self):
        ...
        successful = sum(
            1 for r in self.results if r.get("success") and r.get("status") == "SUCCESS"
        )
        ...
        for result in self.results:
            status_icon = (
                "✓" if result.get("status") == "SUCCESS" else "⚠" if result.get("success") else "✗"
            )
            print(f"\n{status_icon} {result.get('api_name')}:")
            print(f"   Status: {result.get('status')}")
            if result.get("success"):
                print(
                    f"   Valid Records: {result.get('valid_records')}/{result.get('expected_records')}"
                )
                print(f"   Issues: {result.get('issues', 'None')}")
            else:
