...
async def test_endpoint(
    ...
    try:
        ...
        results.add_pass(
            test_name,
            f"Source: {source}, Fields: {list(data.keys())}"
        )
...
async def test_fallback_behavior(client: httpx.AsyncClient) -> None:
    ...
    results.add_warning(
        "Fallback Behavior",
        "Manual test required - simulate HF down and verify fallback works"
    )
    ...
    if response.status_code == 200:
        ...
        if 'cache_ttl_seconds' in meta:
            ...
            if isinstance(ttl, int) and ttl > 0:
                results.add_pass(
                    "Cache TTL",
                    f"TTL set to {ttl} seconds"
                )
            else:
                results.add_warning(
                    "Cache TTL",
                    f"Invalid TTL value: {ttl}"
                )
        else:
            results.add_warning(
                "Cache TTL",
                "cache_ttl_seconds not present in meta"
            )
    else:
