...
class LogManager:
    ...
    def filter_logs(
        ...
        if start_time:
            filtered = [
                log for log in filtered if datetime.fromisoformat(log.timestamp) >= start_time
            ]

        if end_time:
            filtered = [
                log for log in filtered if datetime.fromisoformat(log.timestamp) <= end_time
            ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for log in self.logs:
            ...
            if log.provider_id:
                stats["by_provider"][log.provider_id] = (
                    stats["by_provider"].get(log.provider_id, 0) + 1
                )
    ...
    def search_logs(self, query: str, limit: int = 100) -> List[LogEntry]:
        ...
        for log in reversed(self.logs):
            if (
                ...
                or (log.provider_id and query_lower in log.provider_id.lower())
                or (log.error and query_lower in log.error.lower())
            ):
                results.append(log)
...
if __name__ == "__main__":
    ...
    log_info(
        LogCategory.PROVIDER, "Provider health check", provider_id="coingecko", response_time=234.5
    )
