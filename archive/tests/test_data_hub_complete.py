    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('data'):
                    candle = data['data'][-1]
                    print(f"   آخرین کندل: O:{candle.get('open', 0):,.2f} H:{candle.get('high', 0):,.2f} L:{candle.get('low', 0):,.2f} C:{candle.get('close', 0):,.2f}")
            else:
                print(f"❌ خطا: {response.status_code}")
        except Exception as e:
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and data.get('current'):
                    ...
                    print(f"✅ Fear & Greed Index: {current.get('value')} ({current.get('value_classification')})")
            else:
                print(f"❌ خطا: {response.status_code}")
        except Exception as e:
        ...
        try:
            response = await client.post(
                ...
                json={"text": "Bitcoin price is surging and breaking all resistance levels!", "source": "huggingface"}
            )
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
                    sentiment = data.get('data', {})
                    print(f"✅ تحلیل احساسات: {sentiment.get('label')} (confidence: {sentiment.get('score', 0):.2f})")
            else:
                print(f"❌ خطا: {response.status_code}")
        except Exception as e:
                ...
                if response.status_code == 200:
                    ...
                    if data.get('success'):
                        ...
                        if isinstance(gas, dict) and 'gas_prices' in gas:
                            ...
                            print(f"✅ Gas prices for {chain}: Fast:{prices.get('fast')} Standard:{prices.get('standard')} Slow:{prices.get('slow')}")
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('data'):
                    print(f"   تعداد تراکنش‌های بزرگ: {len(data.get('data', {}).get('transactions', []))}")
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    if posts:
                        post = posts[0]
                        print(f"   پست برتر: {post.get('title', '')[:60]}... (Score: {post.get('score', 0)})")
            else:
                print(f"⚠️ وضعیت: {response.status_code}")
        except Exception as e:
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    for source, state in status.items():
                        icon = "✅" if state == "operational" else "⚠️" if state == "degraded" else "❌"
