...
async def test_endpoint(session: aiohttp.ClientSession, method: str, endpoint: str, **kwargs) -> tuple:
...
async def run_tests():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        success, data, status = await test_endpoint(session, 'GET', '/api/market/ohlc', params={'symbol': 'BTC', 'interval': 60, 'limit': 10})
        ...
        success, data, status = await test_endpoint(session, 'GET', '/api/market/depth', params={'symbol': 'BTC', 'limit': 10})
        if success and 'bids' in data and 'asks' in data:
            log_test("GET /api/market/depth", "PASS", f"Got orderbook with {len(data['bids'])} bids, {len(data['asks'])} asks")
        else:
            log_test("GET /api/market/depth", "FAIL", f"Status: {status}")
        
        success, data, status = await test_endpoint(session, 'GET', '/api/market/tickers', params={'limit': 10})
        ...
        success, data, status = await test_endpoint(
            session, 'POST', '/api/models/test_model/predict',
            json={'symbol': 'BTC', 'context': 'test', 'params': {}}
        )
        signal_id = None
        if success and 'id' in data:
            signal_id = data['id']
            log_test("POST /api/models/{model_key}/predict", "PASS", f"Generated signal: {data['type']} with score {data['score']}")
        else:
            log_test("POST /api/models/{model_key}/predict", "FAIL", f"Status: {status}")
        
        success, data, status = await test_endpoint(
            session, 'POST', '/api/models/batch/predict',
            json={'symbols': ['BTC', 'ETH', 'BNB']}
        )
        if success and 'predictions' in data:
            log_test("POST /api/models/batch/predict", "PASS", f"Generated {len(data['predictions'])} predictions")
        else:
            log_test("POST /api/models/batch/predict", "FAIL", f"Status: {status}")
        
        success, data, status = await test_endpoint(session, 'GET', '/api/signals', params={'limit': 10})
        ...
        success, data, status = await test_endpoint(session, 'GET', '/api/news', params={'limit': 5})
        news_id = None
        ...
        success, data, status = await test_endpoint(
            session, 'POST', '/api/news/analyze',
            json={'text': 'Bitcoin reaches new all-time high'}
        )
        if success and 'sentiment' in data:
            log_test("POST /api/news/analyze", "PASS", f"Sentiment: {data['sentiment'].get('label')}")
        else:
            log_test("POST /api/news/analyze", "FAIL", f"Status: {status}")
        
        success, data, status = await test_endpoint(
            session, 'POST', '/api/sentiment/analyze',
            json={'text': 'Crypto market is bullish today!', 'mode': 'crypto'}
        )
        if success and 'label' in data:
            log_test("POST /api/sentiment/analyze", "PASS", f"Label: {data['label']}, Score: {data['score']:.2f}")
        else:
        ...
        success, data, status = await test_endpoint(
            session, 'GET', '/api/crypto/whales/transactions',
            params={'limit': 10, 'min_amount_usd': 100000}
        )
        if success and 'transactions' in data:
            log_test("GET /api/crypto/whales/transactions", "PASS", f"Retrieved {len(data['transactions'])} whale transactions")
        else:
            log_test("GET /api/crypto/whales/transactions", "FAIL", f"Status: {status}")
        
        success, data, status = await test_endpoint(
            session, 'GET', '/api/crypto/whales/stats',
            params={'hours': 24}
        )
        if success and 'total_transactions' in data:
            log_test("GET /api/crypto/whales/stats", "PASS", f"Total: {data['total_transactions']}, Volume: ${data['total_volume_usd']:,.0f}")
        else:
        ...
        success, data, status = await test_endpoint(
            session, 'GET', '/api/crypto/blockchain/gas',
            params={'chain': 'ethereum'}
        )
        if success and 'gas_prices' in data:
            gas = data['gas_prices']
            log_test("GET /api/crypto/blockchain/gas", "PASS", f"Fast: {gas['fast']:.1f}, Standard: {gas['standard']:.1f}, Slow: {gas['slow']:.1f}")
        else:
            log_test("GET /api/crypto/blockchain/gas", "FAIL", f"Status: {status}")
        
        success, data, status = await test_endpoint(
            session, 'GET', '/api/crypto/blockchain/stats',
            params={'chain': 'ethereum', 'hours': 24}
        )
        if success and 'blocks_24h' in data:
            log_test("GET /api/crypto/blockchain/stats", "PASS", f"Blocks: {data['blocks_24h']}, Txs: {data['transactions_24h']}")
        else:
        ...
        success, data, status = await test_endpoint(
            session, 'POST', '/api/v2/export/signals',
            params={'format': 'json'}
        )
        if success and 'status' in data:
            log_test("POST /api/v2/export/{type}", "PASS", f"Exported {data.get('records', 0)} records")
        else:
        ...
        if success and 'summary' in data:
            ...
            log_test("POST /api/diagnostics/run", "PASS", f"Passed: {summary['passed']}/{summary['total_tests']} ({summary['success_rate']}%)")
        else:
