...
from fastapi import FastAPI, HTTPException, Query, Request, Header, WebSocket, WebSocketDisconnect, Body
...
async def acknowledge_signal(
    request: Request,
    ack_request: SignalAckRequest
):
    """تایید دریافت سیگنال"""
    return {
        "status": "acknowledged",
        "signal_id": ack_request.id
    }
...
async def analyze_news(
    request: Request,
    analyze_request: NewsAnalyzeRequest
):
    """تحلیل محتوای خبر"""
    return {
...
async def analyze_sentiment(
    request: Request,
    sentiment_request: SentimentAnalyzeRequest
):
    """تحلیل احساسات"""
...
async def get_gas_price(
    request: Request,
    chain: str = Query(..., regex="^(ethereum|bsc|polygon)$")
):
    """قیمت Gas"""
    try:
        client = await get_api_client()
...
async def get_providers(request: Request):
    ...
    providers_list.append({
        ...
        "base_url": "internal",
        "capabilities": ["all"],
        "status": "online" if health.get("hf", {}).get("circuit_breaker", {}).get("state") == "closed" else "degraded",
...
async def get_status(request: Request):
    ...
    return {
        ...
        "hf_status": "online" if hf_state == "closed" else "degraded" if hf_state == "half_open" else "offline"
    }
...
def save_market_price(self, symbol, price_usd, market_cap=None, volume_24h=None, price_change_24h=None, source="hf"):
...
def save_whale_transaction(self, blockchain, transaction_hash, from_address, to_address, amount, amount_usd, timestamp, source):
...
def save_gas_price(self, blockchain, gas_price_gwei, fast_gas_price=None, standard_gas_price=None, slow_gas_price=None, source="hf"):
