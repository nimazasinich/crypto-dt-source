...
class ResourceManager:
        ...
        try:
            ...
            if self.config_file.exists():
                backup_file = (
                    ...
                    / f"{self.config_file.stem}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                )
    ...
    def add_provider(self, provider_data: Dict[str, Any]):
        ...
        provider_id = provider_data.get("id") or provider_data.get("name", "").lower().replace(
            " ", "_"
        )
    ...
    def _convert_unified_format(self, registry_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for market in registry_data.get("market_data_apis", []):
            ...
            converted[provider_id] = {
                ...
                "free": market.get("role", "").endswith("_free")
                or market["auth"]["type"] == "none",
            }
    ...
    def validate_provider(self, provider_data: Dict[str, Any]) -> tuple[bool, str]:
        ...
        if not isinstance(provider_data.get("base_url"), str) or not provider_data[
            "base_url"
